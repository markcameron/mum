...
function sendInquiryForm($formData, $config){
    $validated = GUMP::is_valid($formData, array(
        ...
        'inquiry-date-check-in' => 'required',
        'inquiry-time-check-in' => 'required',
        'inquiry-date-check-out' => 'required',
        'inquiry-time-check-out' => 'required',
        'inquiry-adults' => 'required',
        'inquiry-children' => 'required',
        'first-name' => 'required',
        'last-name' => 'required',
        'address' => 'required',
        'zip-code' => 'required',
...
function sendMailchimp($formData, $config){
    ...
    if ($validated === true) {
        ...
        $name = $formData['newsletter-name']; //replace with a test email

        try {
            $subscriber = $Mailchimp_Lists->subscribe($config['mailchimp_list_id'], array('email' => $email, 'name' => $name)); //pass the list id and email to mailchimp
        } catch (Exception $e) {
